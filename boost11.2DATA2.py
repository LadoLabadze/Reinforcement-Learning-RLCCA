...
data = pd.read_csv("SPYUSUSD5years.csv")
    ...
    while i < len(data):
        ...
        if action in ['Buy', 'Sell']:
            ...
            if confidence_ratio >= 0.6:
                capital_used = 1.0 * CapitalUSD  # Strong signal
            elif confidence_ratio >= 0.5:
                capital_used = 0.75 * CapitalUSD  # High confidence
            elif confidence_ratio >= 0.4:
                capital_used = 0.6 * CapitalUSD  # Medium-high
            elif confidence_ratio >= 0.3:
                capital_used = 0.45 * CapitalUSD  # Medium
            elif confidence_ratio >= 0.2:
                capital_used = 0.35 * CapitalUSD  # Low
            elif confidence_ratio >= 0.1:
                capital_used = 0.25 * CapitalUSD  # Very low
            else:
                capital_used = 0.1 * CapitalUSD  # Minimum exposure
        else:
            # Hold action — assume cautious position
            capital_used = 0.15 * CapitalUSD
        ...
        # --- Sell Logic ---
            ...
            effective_entry_price = entry_price + spread  # Buy price slightly worse
            effective_exit_price = current_price - spread  # Sell price slightly worse

            exit_price = current_price
            exit_time = timestamp
            exit_index = i
            price_change = effective_exit_price - effective_entry_price

            commission = (entry_capital / current_price) * (0.005 + 0.01)
            profit = (price_change / effective_entry_price) * entry_capital - commission
            total_profit += profit
...
# Save to Excel
horizontal_summaryG.to_excel("11.2.xlsx", index=False)
...
# Define SPY average annual return
spy_avg_return = 9  # percent

# Calculate average performance from simulations
sim_avg_return = horizontal_summaryG["ProfitPercent"].mean()

# Create the bar plot
plt.figure(figsize=(12, 6))
sns.barplot(data=horizontal_summaryG, x="Simulation", y="ProfitPercent", palette="coolwarm")

# Add SPY average return line (gray)
plt.axhline(spy_avg_return, color='gray', linestyle='--', linewidth=2, alpha=0.6)
plt.text(len(horizontal_summaryG) - 1, spy_avg_return + 0.5, "SPY Avg Return (9%)",
         color='gray', fontsize=10, ha='right', style='italic')

# Add Simulation average return line (red)
plt.axhline(sim_avg_return, color='red', linestyle='--', linewidth=2, alpha=0.8)
plt.text(len(horizontal_summaryG) - 1, sim_avg_return + 0.5,
         f"Model Avg Return ({sim_avg_return:.2f}%)",
         color='red', fontsize=10, ha='right', style='italic')

# Annotate bars
for i, row in horizontal_summaryG.iterrows():
    plt.text(i, row['ProfitPercent'] + 0.5, f"{row['ProfitPercent']:.2f}%",
             ha='center', va='bottom', fontsize=8)

# Titles and labels
plt.title("Profit % by Simulation vs SPY and Model Average Returns", fontsize=14)
